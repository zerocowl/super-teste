...
export default class Order {
    ...
    * generatorOutput(param) {
        let output = ""
        let { entree, side, drink, dessert } = param
        yield output += `${entree.name}, `;
        yield output += ` ${side.name}(x${side.quantity}), `;
        yield output += `${drink.name}(x${drink.quantity}), `;
        yield output += dessert.name;
    }

    static formatOrder(order) {
        let { entree, side, drink, dessert } = order
        let output = "";

        const checkQuantity = (param) => {
            if (!!param.quantity && param.quantity >= 1) {
                let out = Order.generatorOutput(param)
                if (!out.next().done) { return checkQuantity }
            }
        }

        /*   const outputTemplate = (entree) => (side) => (drink) => (dessert) => {
               return `${entree.name}, ${side.name}(x${side.quantity}),
                            ${drink.name}(x${drink.quantity}),` + dessert.name
           }
   */
        const ck = checkQuantity(entree)
        console.log(ck)
        /*
